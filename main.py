        ...
        for item in self.mp4:
            ...
            if str(self.mp4[item][5][0]) in finished:
                ...
                while True:
                    ...
                    # START
                    # 临时解决方案
                    # 经过分析后，现如今超星修改了ohterInfo参数
                    # 加密方法暂时未发现
                    # 采用请求内容卡片的方式暂时解决
                    num = 0
                    cardUrl = 'https://mooc1.chaoxing.com/knowledge/cards?clazzid='+ str(self.clazzid) +\
                              '&courseid='+ str(self.courseid) +\
                              '&knowledgeid='+ str(self.chapterids[num]) +\
                              '&num=0&ut=s&cpi=' + str(self.cpi) + \
                              '&v=20160407-1'
                    newCode = session.get(cardUrl)
                    newCodeList = re.findall(r'"otherInfo":"(.*)","is', newCode.text.replace('\n', '').replace('\t', ''))
                    # print(newCode.text)
                    if len(newCodeList) > 0:
                        pattern = newCodeList[0]
                        print(pattern)
                        num += 1
                        url = 'https://mooc1.chaoxing.com/multimedia/log/a/' + str(self.cpi) + '/' + str(self.mp4[item][1]) + '?clazzId=' + str(self.clazzid) + '&playingTime=' + str(playingtime) + '&duration=' + str(self.mp4[item][2]) + '&clipTime=0_' + str(self.mp4[item][2]) + '&objectId=' + str(self.mp4[item][5][0]) + '&otherInfo=' + pattern + '&jobid=' + str(self.mp4[item][5][1]) + '&userid=' + str(self.userid) + '&isdrag=0&view=pc&enc=' + str(enc) + '&rt=0.9&dtype=Video&_t=' + str(t)
                    # END
